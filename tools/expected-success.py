...
def main():
    ...
    if args.sample_rate == 1:
        ...
        while cumulative < args.success_rate:
            ...
            cumulative += pmf(
                args.trials, expected_value - deviation, success_probability
            )
            cumulative += pmf(
                args.trials, expected_value + deviation, success_probability
            )
    ...
    print(
        "The test should assert %d-%d successes to succeed in %.4f%% of runs"
        % (
            max(expected_value - deviation, 0),
            min(expected_value + deviation, args.trials),
            cumulative,
        )
    )
...
def get_args():
    ...
    parser.add_argument("-r", "--success-rate", default=0.9999, type=float)
